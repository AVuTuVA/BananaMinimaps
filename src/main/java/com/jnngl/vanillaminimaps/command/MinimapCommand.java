...
public class MinimapCommand extends BrigadierCommand {
  ...
  private boolean saveFailed(Minimap minimap) {
    try {
      getPlugin().playerDataStorage().save(minimap);
      return false;
    } catch (Throwable e) {
      getPlugin().getLogger().log(Level.SEVERE, "Failed to save minimap data.", e);
      return true;
    }
  }

  private int enable(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) {
    int affected = 0;
    ...
    for (ServerPlayer serverPlayer : targets) {
      ...
      try {
        ...
        getPlugin().getLogger().log(Level.SEVERE, "Failed to load minimap data for " + player.getName() + ".", e);
  ...
  private int disable(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) {
    int affected = 0;
    ...
    for (ServerPlayer serverPlayer : targets) {
      ...
      try {
        ...
        getPlugin().getLogger().log(Level.SEVERE, "Failed to save minimap data for " + player.getName() + ".", e);
  ...
  private boolean modifyMarker(CommandSourceStack source,
                               Player player,
                               String markerName,
                               BiConsumer<Minimap, SecondaryMinimapLayer> consumer) {
    Minimap minimap = getPlugin().getPlayerMinimap(player);
    if (minimap == null) {
  ...
  private int changeMarkerIcon(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) throws CommandSyntaxException {
    ...
    for (ServerPlayer serverPlayer : targets) {
      ...
      if (modifyMarker(ctx.getSource(), player, markerName,
          (minimap, marker) -> marker.setRenderer(new MinimapIconRenderer(icon)))) {
        affected++;
      }
    }
  ...
  private int renameMarker(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) {
    String markerName = StringArgumentType.getString(ctx, "name");
  ...
  private int removeMarker(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) {
    String markerName = StringArgumentType.getString(ctx, "name");
  ...
  private int fullscreen(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> targets) {
    int affected = 0;
